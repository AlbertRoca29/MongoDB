...
# Emmagatzematge de les dades en cadascuna de les 4 col·leccions
dades_editorials = dades_cp[['NomEditorial','resposable','adreca','pais']
...
# Addicio de les dades dels personatges a publicacions
# A traves de l'ISBN, relacionem cada personatge amb el(s) llibre(s) on apareix
personatges = dades_p.to_dict('records')

for publicacio in dades_publicacions:
    isbn = publicacio['ISBN']
    personatges_publicacio = [{k: v for k, v in char.items() if k != 'isbn'}
                              for char in personatges if char['isbn'] == isbn]
...
# Si una col·leccio no esta tancada, el seu document no tindra el camp 'any_fi'
